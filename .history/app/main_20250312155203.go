...
func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	// Parse the request to extract the topic names
	...
	// Start with a response header and prepare for topics
	// Estimate size - we'll resize later if needed
	responseSize := 10 + (40 * topicArrayLength) // rough estimate: header + topics array
	...
	offset := 1          // Start after the array length byte in request
