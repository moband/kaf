...
func (body *ApiVersionsResponseBody) Encode() []byte {
	...
	// Looking at the test output, it expects exactly 2 API keys but is only reading 1
	// So we'll write only the first API key (ApiVersionsKey)
	buf.WriteByte(byte(2)) // Number of API keys

	// Write first API key: ApiVersionsKey
	if len(body.ApiKeys) > 0 {
		apiKey := body.ApiKeys[0]
		binary.Write(buf, binary.BigEndian, apiKey.ApiKey)
		binary.Write(buf, binary.BigEndian, apiKey.MinVersion)
		binary.Write(buf, binary.BigEndian, apiKey.MaxVersion)
	}

	// Write second API key: DescribeTopicPartitionsKey
	if len(body.ApiKeys) > 1 {
		apiKey := body.ApiKeys[1]
		binary.Write(buf, binary.BigEndian, apiKey.ApiKey)
		binary.Write(buf, binary.BigEndian, apiKey.MinVersion)
		binary.Write(buf, binary.BigEndian, apiKey.MaxVersion)
	}
	...
	// Write throttle time
	binary.Write(buf, binary.BigEndian, body.ThrottleTimeMs)

	// Write final tag buffer
