...
func SerializeDescribeTopicPartitionsResponse(resp *DescribeTopicPartitionsResponse) ([]byte, error) {
	...
	for _, topic := range resp.Topics {
		...
		// Add topic name - The format appears to require a length value followed by
		// the string and a null terminator
		topicNameBytes := []byte(topic.Name)

		// Add string length (2 bytes) - matching the size seen in tester output
		strLenBytes := make([]byte, 2)
		strLen := len(topicNameBytes)
		binary.BigEndian.PutUint16(strLenBytes, uint16(strLen))
