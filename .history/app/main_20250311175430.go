...
func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	...
	fmt.Println("Processing DescribeTopicPartitions for topic:", describeRequest.TopicNames)
	...
	responseSize := 4 // Correlation ID (int32)
	responseSize += 4 // Throttle time (int32)
	responseSize += 2 // Topics array length (int16)

	// For each topic in the request, we'll include a response entry
	topicResponses := make([][]byte, len(describeRequest.TopicNames))
	for i, topicName := range describeRequest.TopicNames {
		// Per topic:
		topicNameLen := len(topicName)
		topicSize := 2 + topicNameLen // Topic name length (int16) + topic name
		topicSize += 2                // Error code (int16)
		topicSize += 16               // Topic ID (UUID - 16 bytes of zeros)
		topicSize += 1                // Is internal flag (boolean)
		topicSize += 4                // Empty partitions array (int32 = 0)
		topicSize += 1                // Tagged fields (int8 = 0)

		// Create the topic response
		topicResponse := make([]byte, topicSize)
		offset := 0

		// Topic name length (int16)
		binary.BigEndian.PutUint16(topicResponse[offset:offset+2], uint16(topicNameLen))
		offset += 2

		// Topic name
		copy(topicResponse[offset:offset+topicNameLen], topicName)
		offset += topicNameLen

		// Error code (UNKNOWN_TOPIC_OR_PARTITION)
		binary.BigEndian.PutUint16(topicResponse[offset:offset+2], ErrorUnknownTopicPartition)
		offset += 2

		// Topic ID (null UUID - 16 bytes of zeros)
		for j := 0; j < 16; j++ {
			topicResponse[offset+j] = 0
		}
		offset += 16

		// Is internal flag (false = 0)
		topicResponse[offset] = 0
		offset += 1

		// Empty partitions array (int32 = 0)
		binary.BigEndian.PutUint32(topicResponse[offset:offset+4], 0)
		offset += 4

		// Tagged fields (int8 = 0)
		topicResponse[offset] = 0

		// Store the topic response
		topicResponses[i] = topicResponse
		responseSize += len(topicResponse)
	}

	// Create the full response buffer
	...
	// Topics count
	binary.BigEndian.PutUint16(response[offset:offset+2], uint16(len(describeRequest.TopicNames)))
	offset += 2

	// Copy each topic response into the buffer
	for _, topicResponse := range topicResponses {
		copy(response[offset:offset+len(topicResponse)], topicResponse)
		offset += len(topicResponse)
	}

	// For debugging
	fmt.Println("Response size:", len(response))
	fmt.Println("Response hexdump:")
	for i := 0; i < len(response) && i < 64; i++ { // Print only first 64 bytes
