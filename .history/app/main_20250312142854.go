...
const (
	...
	ErrorUnknownTopicOrPartition uint16 = 3
...
// CompactString represents a Kafka compact string
type CompactString struct {
	length int64
	value  string
}

// CompactNullableString represents a Kafka compact nullable string
...
func (cs CompactString) serialize() []byte {
	...
	// Length including null terminator (+1)
	binary.Write(buf, binary.BigEndian, uint8(len(cs.value)+1))

	// String value
	buf.WriteString(cs.value)
...
func (cns CompactNullableString) serialize() []byte {
	...
	// Length including null terminator (+1)
	binary.Write(buf, binary.BigEndian, uint8(len(cns.value)+1))
...
// We'll define a simple in-memory topic store for the server
var parsedTopics = map[string]Topic{
	"test-topic": {
		errorCode: 0,
...
func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	...
	fmt.Println("Requested topics:", topicNames)
	...
	if len(relevantTopics) == 0 {
		responseBody = DescribeTopicPartitionsResponse{
			...
			topics: []Topic{
				{
					errorCode: int16(ErrorUnknownTopicOrPartition),
					name: CompactNullableString{
	...
	return sendRawResponse(conn, serializedResponse)
}
...
func parseTopicNames(buf []byte) []string {
	...
	// Try to read length as varint first
	lenOfTopics, err := binary.ReadUvarint(bufReader)
	if err != nil {
		fmt.Println("Error parsing len of topics (in: parseTopicName): ", err)
		// Fallback to reading first byte as length
		if bufReader.Len() > 0 {
			lenOfTopics = uint64(bufReader.Next(1)[0])
		} else {
			return []string{}
		}
	}
	...
	names := []string{}
	// Use actual topics length, not fixed range
	for i := uint64(0); i < lenOfTopics && bufReader.Len() > 0; i++ {
		if bufReader.Len() < 1 {
			break
		}

		lenOfName := int(bufReader.Next(1)[0])
		if lenOfName == 0 || bufReader.Len() < lenOfName {
			break
		}

		name := string(bufReader.Next(lenOfName))
		names = append(names, name)

		// TAG_BUFFER (skip if available)
		if bufReader.Len() > 0 {
			_ = bufReader.Next(1)
		}
	}
...
func (dtpr DescribeTopicPartitionsRequest) parse(buf []byte) {
	// Placeholder implementation - can be expanded if needed
	bufReader := bytes.NewBuffer(buf)

	// Read topics length
	topicsLen, _ := binary.ReadUvarint(bufReader)
	fmt.Println("Topics length:", topicsLen)

	// Future implementation can parse topic names, partition limit, etc.
}
...
func (topic Topic) serialize() []byte {
	...
	// Topic ID
	buf.Write(topic.topicID.serialize())
...
func (partition Partition) serialize() []byte {
	...
	// Replica nodes
	buf.Write(serializeArray(partition.replicaNodes))

	// ISR Nodes
	buf.Write(serializeArray(partition.isrNodes))

	// Eligible Replicas
	buf.Write(serializeArray(partition.eligibleLeaderReplicas))

	// Last Known ELR
	buf.Write(serializeArray(partition.lastKnownElr))

	// Offline Replicas
	buf.Write(serializeArray(partition.offlineReplicas))
...
func serializeArray(arr []int32) []byte {
	buf := new(bytes.Buffer)

	// Array length
	lenOfArr := uint8(len(arr) + 1)
