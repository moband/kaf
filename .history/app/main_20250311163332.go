...
func parseDescribeTopicPartitionsRequest(payload []byte) (*DescribeTopicPartitionsRequest, error) {
	...
	// First read client ID length (2 bytes)
	if len(payload) < offset+2 {
		return nil, fmt.Errorf("invalid payload: truncated before client ID length")
	}
	...
	if len(payload) < offset+clientIDLength {
		return nil, fmt.Errorf("invalid payload: client ID truncated")
	}
	...
	if len(payload) < offset+2 {
		return nil, fmt.Errorf("invalid payload: truncated before topics count")
	}
	topicsCount := int(binary.BigEndian.Uint16(payload[offset : offset+2]))
	offset += 2
	fmt.Printf("Topics count: %d\n", topicsCount)

	// In the hexdump, after the topics count (00 02), we see the byte 12 followed by the topic name
	// This is likely a tag or type identifier for the topic, not the length
	// Let's skip this byte
	offset += 1

	// Now read the topic name which appears to be null-terminated
	// Find the first null byte after the current offset
	topicNameEnd := offset
	...
	topicName := string(payload[offset:topicNameEnd])
	fmt.Printf("Topic name: %s (length: %d)\n", topicName, len(topicName))
