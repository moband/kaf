...
func sendDescribeTopicPartitionsResponse(conn net.Conn, correlationID int32, topicName string) error {
	...
	// Response format for DescribeTopicPartitions
	// Header:
	// - CorrelationID (4 bytes)
	// - Error code (2 bytes)
	// Body:
	// - Throttle time (4 bytes)
	// - Topics array: (first byte is the number of topics, then topics)
	//   - Topic error code (2 bytes)
	//   - Topic name (2 bytes length + string bytes)
	//   - Topic ID (16 bytes for UUID)
	//   - Partitions array (first byte is the number of partitions, then partitions)
	// - _tagged_fields (1 byte)

	// Calculate size
	responseSize := 4 + 2 + 4 + 1 + 2 + 2 + topicNameLen + 16 + 1 + 1
	...
	// CorrelationID (4 bytes)
	binary.BigEndian.PutUint32(response[offset:offset+4], uint32(correlationID))
	offset += 4

	// Error code (2 bytes) - 0 for the overall response
	binary.BigEndian.PutUint16(response[offset:offset+2], ErrorNone)
	offset += 2
	...
	// Topic error code (2 bytes) - 3 for UNKNOWN_TOPIC_OR_PARTITION
	...
	// Topic ID (16 bytes) - all zeros for 00000000-0000-0000-0000-000000000000
	for i := 0; i < 16; i++ {
		response[offset+i] = 0
	}
	offset += 16

	// Number of partitions (1 byte) - 0 for unknown topic
	response[offset] = 0
	offset += 1
