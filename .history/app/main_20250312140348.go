...
func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	...
	for i := 0; i < numTopics; i++ {
		...
		// Read topic name
		topicNames[i] = string(request.Payload[offset : offset+int(topicNameLen)])
		offset += int(topicNameLen)
	}

	// Now construct the response for all topics (all will be unknown topics)
	return sendDescribeTopicPartitionsResponse(conn, request.CorrelationID, topicNames)
}

func sendDescribeTopicPartitionsResponse(conn net.Conn, correlationID int32, topicNames []string) error {
	numTopics := len(topicNames)

	// Calculate the initial response size (without topic data)
	// - CorrelationID (4 bytes)
	// - Error code (2 bytes)
	// - Throttle time (4 bytes)
	// - Topics array length (1 byte)
	// - _tagged_fields (1 byte)
	responseSize := 4 + 2 + 4 + 1 + 1

	// Calculate additional size for each topic
	for _, topicName := range topicNames {
		...
		// - Partitions array length (1 byte)
		responseSize += 2 + 2 + len(topicName) + 16 + 1
	}
	...
	response[offset] = byte(numTopics)
	offset += 1
