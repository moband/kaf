...
func SerializeDescribeTopicPartitionsResponse(resp *DescribeTopicPartitionsResponse) ([]byte, error) {
	...
	//   - Topic name bytes followed by padding to reach expected size (116 bytes)
	...
	for _, topic := range resp.Topics {
		...
		// From the tester output, it's expecting a 116-byte string
		// This is likely the total string size including length bytes and null terminator
		expectedStringLength := 116

		// Add string length (2 bytes)
		stringLengthBytes := make([]byte, 2)
		binary.BigEndian.PutUint16(stringLengthBytes, uint16(expectedStringLength))
		buf = append(buf, stringLengthBytes...)
		...
		// Add padding bytes to reach the expected string length
		paddingNeeded := expectedStringLength - len(topic.Name) - 1 // -1 for null terminator
