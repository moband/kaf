...
func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	...
	// Build response for unknown topic according to DescribeTopicPartitions Response (v0)
	// Reference: https://binspec.org/kafka-describe-topic-partitions-response-v0-unknown-topic

	topicName := describeRequest.TopicName
	topicNameLength := len(topicName)

	// Create buffer to build the response
	// Header size (4 bytes correlation ID) + response
	var bufSize int = 4 // correlation ID

	// Response structure:
	bufSize += 4 // throttle_time_ms (int32)
	bufSize += 2 // topics array length (int16) - always 1

	// For each topic:
	bufSize += 2               // topic name length (int16)
	bufSize += topicNameLength // topic name (string)
	bufSize += 2               // error code (int16)
	bufSize += 16              // topic_id (uuid - 16 bytes)
	bufSize += 1               // is_internal (bool)
	bufSize += 4               // partitions array length (int32) - empty for unknown topic (0)
	bufSize += 1               // tagged_fields (unsigned varint)

	buf := make([]byte, bufSize)
	var offset int = 0

	// Correlation ID (from request)
	binary.BigEndian.PutUint32(buf[offset:offset+4], uint32(request.CorrelationID))
	offset += 4
	...
	// Topic name length
	binary.BigEndian.PutUint16(buf[offset:offset+2], uint16(topicNameLength))
	offset += 2

	// Topic name
	copy(buf[offset:offset+topicNameLength], topicName)
	offset += topicNameLength

	// Error code (UNKNOWN_TOPIC_OR_PARTITION = 3)
	binary.BigEndian.PutUint16(buf[offset:offset+2], ErrorUnknownTopicPartition)
	offset += 2

	// Topic ID (null UUID - 16 bytes of zeros)
	for i := 0; i < 16; i++ {
		buf[offset+i] = 0
	}
	offset += 16
	...
	// Partitions array length - empty (0)
	binary.BigEndian.PutUint32(buf[offset:offset+4], 0)
	offset += 4
	...
	return sendRawResponse(conn, buf)
}
...
func parseDescribeTopicPartitionsRequest(payload []byte) (*DescribeTopicPartitionsRequest, error) {
	// Let's work with the hexdump of an actual request:
	// 00 00 00 31 00 4b 00 00 3d 5e a9 89 00 0c 6b 61
	...
	// Looking directly at the hexdump, the topic name appears to start
	// after "kafka-tester\0\0\2\18" and ends before the next \0
	// Let's try to locate the known pattern of "unknown-topic-" in the payload

	searchPattern := "unknown-topic-"
	foundIndex := -1

	for i := 0; i < len(payload)-len(searchPattern); i++ {
		if string(payload[i:i+len(searchPattern)]) == searchPattern {
			foundIndex = i
			break
		}
	}
	...
	if foundIndex == -1 {
		return nil, fmt.Errorf("invalid payload: could not find topic name pattern")
	}

	// Now find the end of the topic name (null terminator)
	topicNameEnd := foundIndex
	for topicNameEnd < len(payload) && payload[topicNameEnd] != 0 {
		topicNameEnd++
	}

	if topicNameEnd >= len(payload) {
		return nil, fmt.Errorf("invalid payload: topic name not null-terminated")
	}

	// The full topic name is from foundIndex to topicNameEnd
	topicName := string(payload[foundIndex:topicNameEnd])
