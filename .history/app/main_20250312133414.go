...
func parseDescribeTopicPartitionsRequest(payload []byte) (*DescribeTopicPartitionsRequest, error) {
	...
	// - 00 02 - topics count
	// - 12 - possibly a string type marker or length
	// - Topic name ("unknown-topic-xxx")
	// - 00 - NUL terminator for the string
	// - 00 00 00 01 ff 00 - trailing bytes (likely limit, cursor, etc.)
	...
	for i := 0; i < topicsCount; i++ {
		// There appears to be a single byte that represents the type marker
		if len(payload) < offset+1 {
			return nil, fmt.Errorf("payload too short for topic type marker")
		}

		typeMarker := payload[offset]
		offset += 1

		// Find the next null byte to determine end of topic name
		topicStartOffset := offset
		...
		topicName := string(payload[topicStartOffset:offset])
		fmt.Printf("Topic %d: %s (type marker: 0x%02x)\n", i+1, topicName, typeMarker)
	...
	// Process the rest of the payload if available (limit, cursor)
...
func SerializeDescribeTopicPartitionsResponse(response *DescribeTopicPartitionsResponse) []byte {
	...
	for _, topic := range response.Topics {
		// For topic name, use the correct format:
		// First 2 bytes: string length including the NUL terminator
	...
	for _, topic := range response.Topics {
		...
		// Error code - make sure it's correctly set to 3 (UNKNOWN_TOPIC_OR_PARTITION)
