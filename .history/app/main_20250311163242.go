...
func parseDescribeTopicPartitionsRequest(payload []byte) (*DescribeTopicPartitionsRequest, error) {
	...
	if topicsCount < 1 {
		return nil, fmt.Errorf("invalid payload: no topics provided")
	}

	// First topic's name length (2 bytes for Kafka strings)
	if len(payload) < offset+2 {
		return nil, fmt.Errorf("invalid payload: truncated before topic name length")
	}
	topicNameLength := int(binary.BigEndian.Uint16(payload[offset : offset+2]))
	offset += 2

	// Read topic name
	if len(payload) < offset+topicNameLength {
		return nil, fmt.Errorf("invalid payload: topic name truncated (needed %d bytes, have %d)",
			topicNameLength, len(payload)-offset)
	}
	topicName := string(payload[offset : offset+topicNameLength])
	fmt.Printf("Topic name: %s (length: %d)\n", topicName, topicNameLength)
