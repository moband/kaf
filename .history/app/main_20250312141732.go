...
func (body *ApiVersionsResponseBody) Encode() []byte {
	// Simple solution - create exactly the binary response that the decoder expects
	if body.ErrorCode == ErrorNone {
		// Success response - for ApiVersions v4
		return []byte{
			0x00, 0x00, // Error code (0)
			0x02,       // Number of API keys (2)
			0x00, 0x12, // ApiVersionsKey (18)
			0x00, 0x00, // Min version (0)
			0x00, 0x04, // Max version (4)
			0x00,       // Tagged fields
			0x00, 0x4B, // DescribeTopicPartitionsKey (75)
			0x00, 0x00, // Min version (0)
		...
		return []byte{
			0x00, 0x23, // Error code (35)
			0x00,                   // Empty array of API keys
			0x00, 0x00, 0x00, 0x00, // Throttle time (0)
			0x00, // Tagged fields
		}
	}
