...
func readRequest(conn net.Conn) (*KafkaRequest, error) {
	...
	// Read message length
	request.Length = request.ReadInt32()
...
func (r *Response) AppendToBody(data any) *Response {

	binary.Write(r.body, binary.BigEndian, data)

	return r

}

func (r *Response) AppendUvarint(data uint64) *Response {

	buf := make([]byte, binary.MaxVarintLen64)

	numBytes := binary.PutUvarint(buf, data)

	r.body.Write(buf[:numBytes])

	return r

}

func (r *Response) AppendCompactString(data string) *Response {

	r.AppendUvarint(uint64(len(data) + 1))

	r.body.Write([]byte(data))
...
func (r *Response) Bytes() []byte {
	...
	return append(buf, r.body.Bytes()...)

}

func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	req := &DescribeTopicPartitionsRequest{}
	topicCount := request.ReadUVarInt()

	for i := 0; i < int(topicCount); i++ {
		req.Topics = append(req.Topics, request.ReadCompactString())
		request.ReadUVarInt()
	}

	req.ResponsePartitionLimit = request.ReadInt32()

	if request.ReadSingleByte() != 0xff {
		req.Cursor = &PartitionCursor{
			TopicName:      request.ReadCompactString(),
			PartitionIndex: request.ReadInt32(),
		}
	}

	res := models.NewResponse(request.CorrelationID)
	...
	for _, topic := range req.Topics {
		res.AppendToBody(api.UNKNOWN_TOPIC_OR_PARTITION) // Error code
