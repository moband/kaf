...
func parseDescribeTopicPartitionsRequest(payload []byte) (*DescribeTopicPartitionsRequest, error) {
	...
	// Read topic count (2 bytes)
	if len(payload) < offset+2 {
		return nil, fmt.Errorf("invalid payload: truncated before topic count")
	}
	topicsCount := int(binary.BigEndian.Uint16(payload[offset : offset+2]))
	offset += 2

	fmt.Printf("Parsing request with %d topics\n", topicsCount)

	if topicsCount < 1 {
		return nil, fmt.Errorf("invalid payload: no topics provided")
	}

	// Read first topic name length (1 byte)
	if len(payload) < offset+1 {
		return nil, fmt.Errorf("invalid payload: truncated before topic name length")
	}
	topicNameLength := int(payload[offset])
	offset += 1
