...
func (body *ApiVersionsResponseBody) Encode() []byte {
	...
	// Write error code
	binary.Write(buf, binary.BigEndian, body.ErrorCode)

	// Write number of API keys
	// The test expects exactly 2 API keys
	buf.WriteByte(2) // Number of API keys (hardcoded to 2)

	// Write ApiVersionsKey (18)
	binary.Write(buf, binary.BigEndian, ApiVersionsKey)
	binary.Write(buf, binary.BigEndian, ApiVersionsMinVersion)
	binary.Write(buf, binary.BigEndian, ApiVersionsMaxVersion)

	// Write DescribeTopicPartitionsKey (75)
	binary.Write(buf, binary.BigEndian, DescribeTopicPartitionsKey)
	binary.Write(buf, binary.BigEndian, DescribeTopicMinVersion)
	binary.Write(buf, binary.BigEndian, DescribeTopicMaxVersion)

	// Write throttle time (0)
	binary.Write(buf, binary.BigEndian, int32(0))

	// Write final tag buffer (0)
	buf.WriteByte(0)
	...
	for {
		...
		if request.Header.RequestApiKey == ApiVersionsKey {
			...
			responseBody := &ApiVersionsResponseBody{
				ErrorCode: errorCode,
				ApiKeys: []ApiVersionKey{
					{ApiKey: ApiVersionsKey, MinVersion: ApiVersionsMinVersion, MaxVersion: ApiVersionsMaxVersion},
					{ApiKey: DescribeTopicPartitionsKey, MinVersion: DescribeTopicMinVersion, MaxVersion: DescribeTopicMaxVersion},
				},
				ThrottleTimeMs: 0,
				TagBuffer:      0,
			}
