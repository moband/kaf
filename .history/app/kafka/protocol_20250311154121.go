...
func WriteAPIVersionsResponse(writer io.Writer, response *APIVersionsResponse) error {
	...
	size := 2 + 1 // ErrorCode (2 bytes) + API Key Count (1 byte)

	// Add size for each API key (6 bytes each: 2 for key, 2 for min, 2 for max)
	size += 6 * len(response.APIKeys)

	// Add 5 bytes for the throttle time (4) and tagged fields (1)
	size += 5

	// Write the size of the response (excluding the size field itself)
	if err := binary.Write(writer, binary.BigEndian, int32(size)); err != nil {
		return err
	}
	...
	// Write API key count
	if err := binary.Write(writer, binary.BigEndian, byte(len(response.APIKeys))); err != nil {
		return err
	}
	...
	// Write final tagged fields (0 for now)
	if err := binary.Write(writer, binary.BigEndian, byte(0)); err != nil {
		return err
	}
