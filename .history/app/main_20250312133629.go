...
func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	...
	for i, topicName := range describeRequest.TopicNames {
		// Prepare topic response - Unknown topic error code = 3
...
func SerializeDescribeTopicPartitionsResponse(resp *DescribeTopicPartitionsResponse) ([]byte, error) {
	...
	//   - 1 byte for COMPACT_STRING type tag
	//   - 2 bytes for string length + the topic name bytes + 1 for null terminator (COMPACT_NULLABLE_STRING)
	...
	for _, topic := range resp.Topics {
		// Each topic needs: errorCode(1) + typeTag(1) + stringLength(2) + topicName + nullTerminator(1) +
		// partitionsLength(4) + tagBuffer(1)
	...
	for _, topic := range resp.Topics {
		...
		// Add string type tag (1 byte) - For COMPACT_NULLABLE_STRING
		buf = append(buf, 0x12) // This matches the request format where 0x12 marks a string

		// Add topic name length (2 bytes) - Include space for null terminator
		topicNameLengthBytes := make([]byte, 2)
		topicNameLength := len(topic.Name)
		binary.BigEndian.PutUint16(topicNameLengthBytes, uint16(topicNameLength))
		buf = append(buf, topicNameLengthBytes...)

		// Add topic name
		buf = append(buf, []byte(topic.Name)...)

		// Add null terminator (1 byte)
		buf = append(buf, 0)
