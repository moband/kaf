...
func handleDescribeTopicPartitionsRequest(conn net.Conn, request *KafkaRequest) error {
	...
	fmt.Println("DescribeTopicPartitions request:", describeRequest)
	if err != nil {
	...
	// Calculate sizes
	topicNameLength := len(describeRequest.TopicNames)
	...
	// Topic name
	copy(response[offset:offset+topicNameLength], describeRequest.TopicNames[0])
